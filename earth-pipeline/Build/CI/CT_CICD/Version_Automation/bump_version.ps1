...
if (-not $check) {
    Write-Error "Build Failed. Patch Number has reached 999, Update Minor or Major."
    exit 1
}
else {
...
if ($TagsData.count -eq 0) {
    Write-Host "The version tag $previousVersionTag was not found on the remote, exting..." -ForegroundColor Red
    throw
}
...
# Update Common verson files
updateCommonVersion -csFilePath $srcCommVerPath -finalVersion $finalVersion
updateCommonVersion -csFilePath $infCommVerPath -finalVersion $finalVersion
...
updateNuspecVersion -nuspecPath $infNuspec -finalVersion $finalVersion
