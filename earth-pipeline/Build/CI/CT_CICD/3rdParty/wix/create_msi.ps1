...
param( 
    ...
    [Parameter(Mandatory = $false, Position = 3)][string]$FrameworkPath,
    [Parameter(Mandatory = $false, Position = 4)][string]$TargetPaths
)

$ScriptPath = [System.IO.DirectoryInfo]$PSCommandPath
$ProductTemplate = Resolve-Path (Join-Path $ScriptPath.Parent.FullName "Product.wxs")
$TransformTemplate = Resolve-Path (Join-Path $ScriptPath.Parent.FullName "transform.xslt")
...
if (-Not (Test-Path $ProductTemplate -PathType Leaf)) {
    Write-Error -Message "Could not find the template wxs file (Product.wxs), exiting..." -ErrorAction Stop
}
...
function GenerateWxs {
    [CmdletBinding()]
    param ($BinPath, $Component, $TransformTemplate)
    Write-Host "-t" $TransformTemplate
    return (ExecuteSyncProcess $heat "dir `"$BinPath`" -dr INSTALLFOLDER -t $TransformTemplate -cg DynamicFragment -nologo -ag -scom -sreg -sfrag -suid -srd -out `"$TempOut\$Component\sourceFiles.wxs`" -var `"var.Source`"") -eq 0
}
...
try {
    ...
    $BinPath = Join-Path $RootPath "$Component\Version $Version\Bin\$FrameworkPath"
    if (-Not (Test-Path $BinPath -PathType Container)) {
        Write-Error -Message "Could not find the Bin path for $Component, exiting..."
    }
    Write-Host "Packaging contents in $BinPath ..."

    Write-Host "Generating wix files..."
    $WxsSuccess = GenerateWxs $BinPath $Component $TransformTemplate
    if (!$WxsSuccess) {
    ...
    if (-Not (Test-Path -Path "$TempOut\$Component\sourceFiles.wxs" -PathType Leaf)) {
        Write-Error "Could not find the generated source wxs file, exiting..." -ErrorAction Stop
    }
