...
..EXAMPLE
    D:\NcoverCleanup\Scripts\cleanup.ps1

    This will run the script with default values

.EXAMPLE
    D:\NcoverCleanup\Scripts\cleanup.ps1 -datastoreLocation "C:\ProgramData\ncoverdata" -directoryThresholdGB 1.5
    
    This will run the script with arguments passed for the datastore location and  directory threshold 
#>


# Values referenced from variable group "CTWorkspace_Var"
$datastoreLocation = $env:datastoreLocation
$directoryThresholdGB = [decimal]$env:directoryThresholdGB
...
try {
    ...
    if (Test-Path -Path $datastoreLocation) {
        ...
        if ($datastoreSize -gt $directoryThresholdBytes) {
            ...
            try {
                $logsDirs = Get-ChildItem -Directory -Name $datastoreLocation\Logs
            ...
            try {
                ...
                if (Test-Path -Path $ncoverDir) {
                    ...
                    $items = Get-ChildItem -Exclude $excludedFolders 
                    if ($items.Count -gt 0) {
